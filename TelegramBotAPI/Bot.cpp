...
QVector<QHttpPart> getExtraParts_Photos(const QVector<Telegram::InputMediaPhoto>& photos)
{
    QVector<QHttpPart> extraParts;

    int nPhotoIndex = 0;

    for (const Telegram::InputMediaPhoto& photo : photos) // must be done first to set tg_media_name
    {
        if (std::holds_alternative<QFile*>(photo.media))
        {
            QFile* file = std::get<QFile*>(photo.media);

            QHttpPart part;

            QString photoName = QString("photo_%1").arg(nPhotoIndex++);

            QString header = QStringLiteral("form-data; name=\"%1\"; filename=\"%2\"").arg(photoName).arg(file->fileName());
            part.setHeader(QNetworkRequest::ContentDispositionHeader, header);
            part.setBodyDevice(file);

            file->setProperty("tg_media_name", photoName);

            extraParts.push_back(part);
        }
    }
...
QVector<QHttpPart> getExtraParts_Videos(const QVector<Telegram::InputMediaVideo>& videos)
{
    QVector<QHttpPart> extraParts;
    int nVideoIndex = 0;
    for (const Telegram::InputMediaVideo& video : videos) // must be done first to set tg_media_name
    {
        if (std::holds_alternative<QFile*>(video.media))
        {
            QFile* file = std::get<QFile*>(video.media);
            QHttpPart part;
            QString videoName = QString("video_%1").arg(nVideoIndex++);
            QString header = QStringLiteral("form-data; name=\"%1\"; filename=\"%2\"").arg(videoName).arg(file->fileName());
            part.setHeader(QNetworkRequest::ContentDispositionHeader, header);
            part.setBodyDevice(file);
            file->setProperty("tg_media_name", videoName);
            extraParts.push_back(part);
        }
    }
    return extraParts;
...
std::future<QVector<Telegram::Message>> Telegram::Bot::sendMediaGroup(const std::variant<qint64, QString>& chat_id, 
    const QVector<InputMediaPhoto>& photos, const QVector<InputMediaVideo>& videos, const std::optional<bool>& disable_notification, 
        const std::optional<qint32>& reply_to_message_id, const std::optional<bool>& allow_sending_without_reply) {

    QVector<QHttpPart> extraPartsPhotos = getExtraParts_Photos(photos);
    QVector<QHttpPart> extraPartsVideos = getExtraParts_Videos(videos);
    ...
    QVector< InputMedia *> allMediaFiles;

    for (const InputMediaPhoto& photo : photos)
        allMediaFiles.push_back(const_cast<InputMediaPhoto*>(&photo));
    for (const InputMediaVideo& video : videos)
        allMediaFiles.push_back(const_cast<InputMediaVideo*>(&video));

    std::unique_ptr<QHttpMultiPart>&& request_body = Utility::QHttpMultiPartInserter::make(keys,
        chat_id,
        allMediaFiles,
        disable_notification,
        reply_to_message_id,
    ...
    for (const QHttpPart &part : extraPartsPhotos)
        request_body->append(part);

    for (const QHttpPart& part : extraPartsVideos)
        request_body->append(part);

    return std::async(std::launch::async, [this, request_body = std::move(request_body)]() -> QVector<Message> {

        QByteArray responseBA = Utility::Network::post(request_body.get(), MAKE_LINK_TO_TG_API(bot_settings->bot_token, "sendMediaGroup"));

        if (QJsonObject response = QJsonDocument::fromJson(responseBA).object();
            response["ok"].toBool() == false)
        {
            emit errorOccured(response);
            return {};
