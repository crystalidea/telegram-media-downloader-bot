...
QJsonObject Telegram::InputMediaPhoto::toObject() const {
	...
	{
		QString tg_media_name = std::get<QFile*>(media)->property("tg_media_name").toString();
		
		input_media_photo_json_object.insert("media", QString("attach://%1").arg(tg_media_name));
	}
	else if (std::holds_alternative<QString>(media)) 
		input_media_photo_json_object.insert("media", std::get<QString>(media));
    ...
    input_media_photo_json_object.insert("type", "photo");
